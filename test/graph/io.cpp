...
TEST_CASE("graph I/O", "[graph-io]") {
  ...
  graph g2 = read_graph_tsv<graph>("datasets/karate.tsv.zst");
  REQUIRE(g == g2);

  write_graph_tsv("datasets/karate.out.tsv", g);
  graph g3 = read_graph_tsv<graph>("datasets/karate.out.tsv");
  REQUIRE(g == g3);

  write_graph_binary("datasets/karate.out.bgl", g);
  graph g4 = read_graph_binary<graph>("datasets/karate.out.bgl");
  REQUIRE(g == g4);
